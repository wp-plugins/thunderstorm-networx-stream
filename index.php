...
class thunderstorm_stream_widget_ extends WP_Widget {
    ...
    */
    function thunderstorm_stream_widget_() {
        /* Widget settings. */
        $widget_ops = array( 'classname' => 'thunderstorm_stream_widget', 'description' => 'Ermöglicht das Anzeigen der Aktivität eines Nutzers über die thunderstorm-networx-API' );

        /* Widget control settings. */
        $control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'thunderstorm_stream_widget' );

        /* Create the widget. */
        $this->WP_Widget( 'thunderstorm_stream_widget', 'Ermöglicht das Anzeigen der Aktivität eines Nutzers über die thunderstorm-networx-API', $widget_ops, $control_ops );
    }
...
function thunderstorm_stream_box() {
    echo '<h3 class="widgettitle">thunderstorm-networx Stream</h3>';
    ...
    if ($json->status != 'ok') {
        ...
        foreach ($elements as $element) {
            ...
            echo '<p style="word-break: break-all;"><b><a href="http://www.thunderstorm-networx.de/members/'. $user. '" target="_blank">'. $user. '</a> am '. date('d.m.y \u\m H:i:s', $ptime). '</b><br />'. $content. ' [<a href="'. $link. '" target="_blank">Mehr</a>]</p>';
        }
    }
